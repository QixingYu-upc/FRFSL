...
parser.add_argument("-f","--feature_dim",type = int, default = 40)
parser.add_argument("-c","--src_input_dim",type = int, default = 128)
parser.add_argument("-d","--tar_input_dim",type = int, default = 126) # PaviaU=103；salinas=204
...
test_data = "/mnt/hdd/yqx/DCFSL-2021-main/datasets/Pavia.mat"
test_label = "/mnt/hdd/yqx/DCFSL-2021-main/datasets/Pavia_gt.mat"
...
def get_train_test_loader(Data_Band_Scaler, GroundTruth, class_num, shot_num_per_class):
    ...
    hsi_pic = np.zeros((G.shape[0], G.shape[1], 3))
    ...
    utils.classification_map(hsi_pic[4:-4, 4:-4, :], G[4:-4, 4:-4], 24,  "classificationMap/IP_gt{}shot.png".format(TEST_LSAMPLE_NUM_PER_CLASS))
...
class D_Res_3d_CNN(nn.Module):
    def __init__(self, in_channel, out_channel1, out_channel2):
        ...
        self.conv = nn.Conv3d(in_channels=out_channel2,out_channels=8,kernel_size=3, bias=False)

        self.final_feat_dim = 60
...
def euclidean_metric(a, b):
    n = a.shape[0]
    m = b.shape[0]
    a = a.unsqueeze(1).expand(n, m, -1)#[n,m,feature_dim]
    b = b.unsqueeze(0).expand(n, m, -1)#[n,m,feature_dim]
    logits = -((a - b)**2).sum(dim=2)
    return logits
    
def Get_Distance(query,support):
    #Classification Loss
    beta = torch.tensor([-0.6])#paviaU-0.5k=10,chengliu-0.5k=5,liulin-0.55k=6pavia-0.6,k=3,cihuai-0.5,k=5
    support = support.reshape(CLASS_NUM, SHOT_NUM_PER_CLASS, -1)
    lam = support.size(1)/support.size(2)
    rho = torch.exp(beta).cuda()+1e-6
    st = support.permute(0,2,1)
    sst = support.matmul(st)
    sst_plus_ri = sst + torch.eye(sst.size(-1)).cuda().unsqueeze(0).mul(lam)
    ...
    Q_bar = w.matmul(support).mul(rho)
    ...
    for i in range(way):
        for j in range(i):
            ...
            L2[counter] = j
            counter += 1
    s1 = support.index_select(0,L1)
    s2 = support.index_select(0,L2)
    auxdists = s1.matmul(s2.permute(0,2,1))
    assert auxdists.size(-1)==shot
    frobs = auxdists.pow(2).sum(-1).sum(-1)
    
    
    return -dist,frobs.sum().mul(.03)

def Instance_recalibration(support,query,seed):
    data = torch.cat([support,query])
    data = data.reshape(CLASS_NUM * (QUERY_NUM_PER_CLASS + SHOT_NUM_PER_CLASS), -1)
    X = data.cpu().detach()
    GMM = GaussianMixture(n_components=CLASS_NUM, covariance_type='full', max_iter=200, random_state=seed ,tol=1e-5)
    #GMM = BayesianGaussianMixture(n_components=CLASS_NUM, covariance_type='full', max_iter=200, random_state=seed ,tol=1e-5)
    GMM.means_init = support.cpu().detach()
    GMM.fit(X)
    responsibilities = GMM.predict_proba(X)
    k = 3
    highest_log_likelihood_samples = []
    log_likelihoods = GMM.score_samples(X)
    for cluster_idx in range(GMM.n_components):
        weighted_log_likelihood = responsibilities[:, cluster_idx] * log_likelihoods
        top_k_indices = np.argsort(weighted_log_likelihood)[-k:]
        top_k_samples = X[top_k_indices]
        highest_log_likelihood_samples.append(top_k_samples)
    samples_top_k = np.concatenate([np.array(lst) for lst in highest_log_likelihood_samples],axis=0)
    samples_top_k = torch.from_numpy(samples_top_k).cuda().float()
    support = support.reshape(CLASS_NUM * SHOT_NUM_PER_CLASS, -1)
    weight = euclidean_metric(support,samples_top_k)
    weight = -1./weight.mean(1).unsqueeze(1).expand(weight.shape[0],FEATURE_DIM)
    weight_support = weight * support
...
for iDataSet in range(nDataSet):
    ...
    domain_classifier = D.GOT(FEATURE_DIM,FEATURE_DIM)
    ...
    for episode in range(EPISODE):
        ...
        if episode % 2 == 0:
            ...
            '''
            flops, params = profile(feature_encoder, (supports.cuda(),))
            print('flops: ', flops, 'params: ', params)
            print('flops: %.9f M, params: %.9f M' % (flops / 1000000.0, params / 1000000.0))
            '''
            # Prototype network
            ...
            '''原型'''
            support_proto = Instance_recalibration(support_proto,query_features,seeds[iDataSet])
            logits,aux_loss = Get_Distance(query_features,support_proto)
            ...
            domain_loss = domain_classifier(X.transpose(1,2), Y.transpose(1,2))
            #torch.cuda.synchronize()  # 等待GPU操作完成
            #end = time.perf_counter()
            #print(f"GPU同步后运行时间: {end - start:.6f} 秒")
            '''
            flops, params = profile(domain_classifier, (X.transpose(1,2),Y.transpose(1,2),))
            print('flops: ', flops, 'params: ', params)
            print('flops: %.9f M, params: %.9f M' % (flops / 1000000.0, params / 1000000.0))
            '''
            loss = f_loss + aux_loss + domain_loss  # 0.01
        ...
        # target domain few-shot + domain adaptation
            ...
            #end = time.perf_counter()
            #print(f"GPU同步后运行时间: {end - start:.6f} 秒")
            loss = f_loss + aux_loss + domain_loss  # 0.01 0.5=78;0.25=80;0.01=80
        ...
        if (episode + 1) % 100 == 0:  # display
            ...
            print('episode {:>3d}:  domain loss: {:6.4f}, fsl loss: {:6.4f}, auxiliary loss: {:6.4f}, acc {:6.4f}, loss: {:6.4f}'.format(episode + 1, \
                                                                                                                domain_loss.item(),
                                                                                                                f_loss.item(),
                                                                                                                aux_loss.item(),
                                                                                                                total_hit / total_num,
    ...
    print('***********************************************************************************')
    '''
    psne_datas = np.array([],dtype=np.int64)
    psne_features = np.array([], dtype=np.int64)
    psne_truth = np.array([],dtype=np.int64)
    feature_encoder = Network()
    feature_encoder.cuda()
    feature_encoder.load_state_dict(torch.load(str("checkpoints/DFSL_feature_encoder_" + "PC_" + str(iDataSet) + "iter_" + str(TEST_LSAMPLE_NUM_PER_CLASS) + "shot.pkl")))
    for test_datas, test_labels in test_loader:
        batch_size = test_labels.shape[0]
        test_features, _  = feature_encoder(Variable(test_datas).cuda(), domain='target')  # (100, 160)
        max_value = test_features.max()  # 89.67885
        min_value = test_features.min()  # -57.92479
        test_features = (test_features - min_value) * 1.0 / (max_value - min_value)
        test_labels = test_labels.numpy()
        datas = test_datas.cpu().detach().numpy()
        datas = np.mean(datas.reshape(test_datas.shape[0],test_datas.shape[1],-1),axis=-1)
        if psne_datas.shape[0]==0:
            psne_datas = datas
        else:
            psne_datas = np.concatenate((psne_datas,datas),axis=0)
            features = test_features.cpu().detach().numpy()
        features = test_features.cpu().detach().numpy()
        if psne_features.shape[0]==0:
            psne_features = features
        else:
            psne_features = np.concatenate((psne_features,features),axis=0)        
        psne_truth = np.append(psne_truth,test_labels)
    draw.main2(psne_datas,psne_truth,'/mnt/hdd/yqx/DCFSL-2021-main/T-SNE/data_PaviaC')
    draw.main2(psne_features,psne_truth,'/mnt/hdd/yqx/DCFSL-2021-main/T-SNE/feature_final_PaviaC_'+str(last_accuracy))
    '''
    del train_loader
    del test_loader
    del target_da_metatrain_data
    del target_loader
    del G,RandPerm
    del Row
    del Column
